...
from prefixTuning import PrefixTuningGPT2
...
class PrefixDialogModule(BaseTransformer):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        if "gpt" not in self.hparams.model_name_or_path:
            raise ValueError(
                "Only GPT-2 backbones are supported. Set --model_name_or_path to a GPT-2 checkpoint "
                "(e.g. gpt2). BART support was removed because prefixBart.py is not shipped."
            )
        self.model_type = "gpt"
    ...
    def _step(self, batch: dict) -> Tuple:
        ...
        tgt_src_ids = torch.full(
            src_ids.size(),
            fill_value=self.pad if not self.training else -100,
            requires_grad=False,
        ).to(src_ids.device)
